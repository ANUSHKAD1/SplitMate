...
function rupeeAmountToInput(amount) { return String(amount ?? '') }
...
function paiseToRupees(amountInPaise) {
  return `${amountInPaise / 100n}.${String(amountInPaise % 100n).padStart(2, '0')}`
}

function normalizedRupeeAmount(value) { return value.trim().replace(/^0+(?=\d)/, '') }
...
function buildInitialState(members, expense) {
  ...
  return {
    description: expense?.description || '', amount: expense ? rupeeAmountToInput(expense.amount) : '', paidBy: String(expense?.paid_by || members[0]?.id || ''),
    ...
    customAmounts: Object.fromEntries(splitUserIds.map((userId) => {
      ...
      return [userId, split ? rupeeAmountToInput(split.amount) : '0.00']
    })),
  }
}
...
export default function ExpenseForm({ members, expense, onSubmit, onCancel, isSubmitting, submitError }) {
  ...
  const amountInPaise = rupeesToPaise(form.amount)
  const selectedMembers = members.filter((member) => form.splitUserIds.includes(member.id))
  const customTotal = useMemo(() => selectedMembers.reduce((total, member) => total + (rupeesToPaise(form.customAmounts[member.id] || '') ?? 0n), 0n), [form.customAmounts, selectedMembers])
  const customHasInvalidAmount = selectedMembers.some((member) => rupeesToPaise(form.customAmounts[member.id] || '') === null)
  ...
  const equalShares = useMemo(() => {
    ...
    const memberCount = BigInt(selectedMembers.length)
    const base = amountInPaise / memberCount, remainder = amountInPaise % memberCount
    return selectedMembers.map((member, index) => ({ ...member, amount: base + (BigInt(index) < remainder ? 1n : 0n) }))
  ...
  return <form onSubmit={handleSubmit} noValidate className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
    ...
    <div className="mt-4 grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-sm font-medium">Amount (INR)<input type="text" pattern="\d+(?:\.\d{1,2})?" value={form.amount} onChange={(event) => updateForm('amount', event.target.value)} inputMode="decimal" placeholder="0.00" className="mt-1 w-full rounded-md border border-slate-300 bg-white px-3 py-2" /></label>
    ...
    <fieldset className="mt-5"><legend className="text-sm font-medium">Split with</legend><div className="mt-2 divide-y divide-slate-200 rounded-md border border-slate-200 bg-white">{members.map((member) => {
      ...
      return <div key={member.id} className="flex items-center justify-between gap-3 px-3 py-2"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={checked} onChange={() => toggleMember(member.id)} />{member.name}</label>{form.splitType === 'custom' && checked && <label className="flex items-center gap-1 text-sm text-slate-600">₹<input type="text" pattern="\d+(?:\.\d{1,2})?" value={form.customAmounts[member.id] ?? '0.00'} onChange={(event) => setForm((current) => ({ ...current, customAmounts: { ...current.customAmounts, [member.id]: event.target.value } }))} inputMode="decimal" className="w-24 rounded border border-slate-300 px-2 py-1" aria-label={`${member.name}'s share in INR`} /></label>}</div>
    })}</div></fieldset>
    {form.splitType === 'equal' && selectedMembers.length > 0 && <div className="mt-4 rounded-md bg-white p-3 text-sm"><p className="font-medium">Calculated shares</p><ul className="mt-2 space-y-1 text-slate-600">{equalShares.map((share) => <li key={share.id}>{share.name}: {formatIndianRupees(paiseToRupees(share.amount))}</li>)}</ul><p className="mt-2 text-xs text-slate-500">This is a preview; the backend validates and assigns the final split.</p></div>}
    {form.splitType === 'custom' && <div className="mt-4 text-sm"><p className="font-medium">Running total: {formatIndianRupees(paiseToRupees(customTotal))}</p>{isCustomMismatch && <p role="alert" className="mt-1 text-red-700">Shares must equal {amountInPaise === null ? 'a valid expense amount' : formatIndianRupees(paiseToRupees(amountInPaise))}.</p>}</div>}
